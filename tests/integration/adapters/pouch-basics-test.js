  ...
  let allTests = function() {
    ...
    test("can find one", function(assert) {
      ...
      Ember.RSVP.Promise
        ...
        .then(found => {
          assert.equal(found.get("id"), "D", "should have found the requested item");
          assert.deepEqual(found.get("flavor"), "black bean", "should have extracted the attributes also");
        })
        .finally(done);
    });
    ...
    test("can find associated records", function(assert) {
      ...
      Ember.RSVP.Promise
        ...
        .then(found => {
          assert.equal(found.get("id"), "C", "should have found the requested item");
          return found.get("ingredients");
        })
        .then(foundIngredients => {
          assert.deepEqual(foundIngredients.mapBy("id"), ["X", "Y"], "should have found both associated items");
    ...
    test("creating an associated record stores a reference to it in the parent", function(assert) {
      ...
      Ember.RSVP.Promise
        ...
        .then(foundIngredients => {
          assert.deepEqual(foundIngredients.mapBy("name"), ["pineapple"], "should have fully loaded the associated items");
        })
        .finally(done);
    });
      ...
      test("update an existing record", function(assert) {
        ...
        Ember.RSVP.Promise
          ...
          .then(updatedDoc => {
            ...
            assert.equal(updatedDoc._rev, recordInStore.get("rev"), "should have associated the ember-data record with the updated rev");
          })
          .finally(done);
      });
    }
