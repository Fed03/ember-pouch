...
test('it can find hasMany relationships', function(assert) {
  return putRaw([
    { _id: 'tacoSoup_2_A', data: { flavor: 'foo', ingredients: ['X', 'Y'] } },
    { _id: 'tacoSoup_2_B', data: { flavor: 'bar', ingredients: ['Z'] } },
    { _id: 'foodItem_2_X', data: { name: 'carrot' } },
    { _id: 'foodItem_2_Y', data: { name: 'tomato' } },
    { _id: 'foodItem_2_Z', data: { name: 'pork' } }
  ]).then(() => {
...
test('it updates parent record with associated record: mode 1', function(assert) {
  return Ember.run(() => {
    return this.store.createRecord('taco-soup', {
      ...
      // Mode 1
      const ingredient = this.store.createRecord('food-item', {
        name: 'carrot'
      });
