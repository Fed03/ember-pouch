...
test('a record that is not loaded stays not loaded when it is changed', function (assert) {
  ...
  return Ember.run(() => {
    return putRaw([
      {_id: 'tacoSoup_2_A', data: { flavor: 'foo' } },
      {_id: 'tacoSoup_2_B', data: { flavor: 'foo' } },
    ]).then(() => {
...
test('a new record is not automatically loaded', function (assert) {
  ...
  return Ember.run(() => {
    ...
    return putRaw({ _id: 'tacoSoup_2_C', data: { flavor: 'sofritas' } });
    }).then(() => {
      return promiseToRunLater(() => {
...
test('a new record is automatically loaded if flag is true', function (assert) {
  ...
  return Ember.run(() => {
    return putRaw({
      ...
      return this.store.findRecord('taco-salad', 'A');
    }).then(soupA => {
      assert.equal('foo', soupA.get('flavor'), 'the loaded instance should reflect the initial test data');
      assert.equal(null, this.store.peekRecord('taco-salad', 'B'), 'test setup: record should not be loaded already');
      ...
      return promiseToRunLater(() => {
        var alreadyLoadedSoupB = this.store.peekRecord('taco-salad', 'B');
